...
`;
  ...
  function initialize() {
    ...
    const licenceElement = getElementById("licence");
    if (licenceElement) {
      licenceElement.addEventListener("change", handleInputChange);
    }
    const detailsInput = getElementById("details");
    if (detailsInput) {
  ...
  async function handleInputChange() {
    ...
    const htmlContent = await parse(markdownContent);
    updatePreview(markdownContent, htmlContent);
  }
  function generateMarkdown() {
    const details = getValue("details").trim();
    const gifUrl = getValue("gif-url").trim();
    const technologies = getValue("technologies").split(",").map((t) => t.trim()).filter(Boolean).map((t) => `- ${t}`).join("\n");
    const dependencies = getValue("dependencies").split(",").map((r) => r.trim()).filter(Boolean).map((r) => `- ${r}`).join("\n");
    const localConfiguration = getValue("local-configuration").trim();
    const testing = getValue("testing").trim();
    const support = getValue("support").trim();
    const licence = getValue("licence");
...
`;
  ...
  function copyToClipboard() {
    const outputTextarea = getElementById("markdown-output");
    if (!outputTextarea)
      return;
  ...
  function updatePreview(markdownContent, htmlContent) {
    ...
    const outputElement = getElementById("markdown-output");
    if (previewElement)
  ...
  function addCodeQuote(inputId) {
    const currentVal = getValue(inputId);
